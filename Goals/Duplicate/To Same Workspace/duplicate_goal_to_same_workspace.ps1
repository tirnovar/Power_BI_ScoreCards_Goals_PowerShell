...
function DuplicateGoal($idScorecard, $sourceScorecard, $duplicates, $sourceGoal) {
    $goalsByParentId = @{}
    $topLevelGoals = @()
    ...
    for (($refreshes = 0); $refreshes -lt $duplicates; $refreshes++) {
        CopyGoal -sourceGoal $sourceGoal -destinationScorecardId $idScorecard -parentGoalId $goalsByParentId
...
function ShowPrompt() {
    while ($true) {
        ...
        Write-Host " [c] - Copy goals to an existing scorecard"
        Write-Host " [q] - Quit"
    ...
    if ($action -eq "c") {
               DuplicateGoal -idScorecard $scorecardId -sourceScorecard $sourceScorecard -duplicates $numberOfDuplicates -sourceGoal $sourceGoal
    }
}
